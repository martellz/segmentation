...
class WaterDataset(Dataset):
  ...
  def __init__(self, root:str, transforms=None, device='cuda', cache_data=True):
    self.root = Path(root)
...
if __name__ == '__main__':
  ...
  from transforms import ColorJitter, RandomRotation, RandomHorizontalFlip, RandomVerticalFlip
  ...
  dataset = WaterDataset('newdata/train_add_label', transforms, 'cpu')
  print('dataset length: {}'.format(len(dataset)))
  image, label = dataset[20]
  plt.imshow(image.squeeze().permute(1, 2, 0))
  plt.show()
  plt.imshow(label.squeeze().permute(1, 2, 0))
  plt.show()
